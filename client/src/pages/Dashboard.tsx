...
const StatCard = ({ 
  ...
  <Card className="bg-slate-900 border-slate-800 p-5 transition-all hover:border-slate-700">
    <div className="flex items-center justify-between mb-4">
      <div className={cn("w-12 h-12 rounded-xl border flex items-center justify-center", colorClass)}>
        <Icon className="w-6 h-6" />
      </div>
      {trend && <div className="px-2.5 py-1 rounded-full text-xs font-semibold bg-slate-800 border border-slate-700">{trend}</div>}
    </div>
    <div>
...
const TransactionItem = ({ trade }: { trade: Trade }) => {
  ...
  return (
    <div className="group flex items-center justify-between p-3.5 rounded-xl bg-slate-800/30 hover:bg-slate-800 border border-transparent hover:border-slate-700 transition-all">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <span className={cn("text-[10px] font-bold px-1.5 py-0.5 rounded", trade.side === 'BUY' ? "text-emerald-400 bg-emerald-950" : "text-rose-400 bg-rose-950")}>
            {trade.side}
          </span>
        </div>
      </div>
      <div className="text-right">
        <p className={cn("text-sm font-bold font-mono", isProfit ? "text-emerald-400" : "text-rose-400")}>
          {isProfit ? '+' : ''}{formatCurrency(pnl)}
        </p>
        <p className="text-slate-500 text-[10px] uppercase font-medium mt-0.5">{trade.status}</p>
      </div>
    </div>
  );
};
...
export default function Dashboard() {
  ...
  const wallets: WalletAsset[] = useMemo(() => [
    { name: "Arbitrage", code: "ARB", balance: userInfo?.balance_arb || 0, color: "bg-purple-500", link: "/arbitrage" },
    { name: "Forex", code: "FX", balance: userInfo?.balance_forex || 0, color: "bg-blue-500", link: "/forex" },
    { name: "Futures", code: "FUT", balance: userInfo?.balance_fut || 0, color: "bg-yellow-500", link: "/futures" },
  ], [userInfo]);
    ...
    return (
      <Layout>
        <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4">
          ...
          <p className="text-slate-400 mb-6">We couldn't fetch your account data. Please check your connection.</p>
          <button onClick={() => window.location.reload()} className="px-6 py-2 bg-emerald-600 rounded-lg text-white font-medium hover:bg-emerald-500">
            Retry
          </button>
        </div>
      </Layout>
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8 py-8 animate-in fade-in duration-500">
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-white">
            {getGreeting()}, <span className="text-emerald-400">{userInfo?.name?.split(' ')[0] || 'Trader'}</span>
          </h1>
          <p className="text-slate-400 mt-1">Here is what's happening with your portfolio today.</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          <StatCard 
            ...
            trend={stats && <span className="text-emerald-400 font-medium">Profit: {formatCurrency(stats.totalProfit)}</span>}
          />
          <StatCard 
            title="Total P&L" 
            ...
            trend={<span className={cn("font-medium", (stats?.totalPnl || 0) >= 0 ? "text-emerald-400" : "text-rose-400")}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 space-y-6">
            <Card className="bg-slate-900 border-slate-800 overflow-hidden">
              <div className="p-6 border-b border-slate-800 flex flex-row justify-between items-center">
                <div>
                  <h2 className="text-lg font-bold text-white">Assets</h2>
                  <p className="text-slate-400 text-sm">Your portfolio distribution</p>
                </div>
                <Link href="/wallet" className="px-4 py-2 bg-slate-800 text-white text-sm font-medium rounded-lg hover:bg-slate-700 border border-slate-700 transition-colors">
                  Manage Wallet
                </Link>
              </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-slate-800">
                    {wallets.map((wallet) => (
                      <tr key={wallet.code} className="group hover:bg-slate-800/40 transition-colors">
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            <div className={cn("w-8 h-8 rounded-lg flex items-center justify-center text-white text-xs font-bold shadow-lg", wallet.color)}>
                              {wallet.code.substring(0, 2)}
                            </div>
                            <div>
                              <p className="text-white font-medium text-sm">{wallet.name}</p>
                              <p className="text-slate-500 text-xs">{wallet.code}</p>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4 text-right text-slate-300 font-mono text-sm">
                          {isLoading ? <Skeleton className="h-4 w-20 ml-auto bg-slate-800" /> : formatCurrency(wallet.balance)}
                        </td>
                        <td className="px-6 py-4 text-right text-white font-bold font-mono text-sm">
                          {isLoading ? <Skeleton className="h-4 w-20 ml-auto bg-slate-800" /> : formatCurrency(wallet.balance)}
                        </td>
                        <td className="px-6 py-4 text-right">
                          <Link href={wallet.link} className="text-emerald-400 text-xs font-bold hover:text-emerald-300 hover:underline">
                            TRADE
                          </Link>
                        </td>
                      </tr>
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <Card className="bg-slate-900 border-slate-800 h-[400px] flex flex-col">
              <div className="p-5 border-b border-slate-800">
                <h3 className="text-white font-bold">Recent Trades</h3>
              </div>
              <div className="p-4 flex-1 overflow-y-auto space-y-2 custom-scrollbar">
                {isLoading ? (
                  Array(5).fill(0).map((_, i) => <Skeleton key={i} className="h-14 w-full bg-slate-800 rounded-xl" />)
                  ...
                  notifications.slice(0, 4).map((n, idx) => (
                    <div key={n.id || idx} className="flex gap-3 group">
                      <div className="flex-shrink-0 mt-1.5">
                        <div className="w-2 h-2 rounded-full bg-emerald-500 ring-4 ring-emerald-500/10 group-hover:ring-emerald-500/20 transition-all"></div>
                      </div>
                      <div>
